...
////------------------------------------------------------- Disconnect From Hub ----------------------------------------////
    ...
    private void OpenConnectionToIotHub() {
        new Thread() {
            public void run() {
                try {
                    client.open();
                } catch (IOException ioException) {
                    if (StringUtils.containsIgnoreCase(ExceptionUtils.getRootCauseMessage(ioException), "TransportException: Timed out waiting to connect to service") && _shouldRetry) {
                        try {
                            Thread.sleep(2000);
                            _shouldRetry = false;
                            OpenConnectionToIotHub();
                    ...
                    emitHelper.logError(getReactContext(), exception);
                    System.err.println("Exception while opening IoTHub connection: " + exception.getMessage());
                    clientBusy = false;
                    _promise.reject(this.getClass().getSimpleName(), exception);
                }
            }
